package concurrency.false_sharing;
 ...
 * <p>
 * Cuando las caches de la CPU estan leyendo datos de caches de nivel inferior o de la RAM (por ejemplo, L1 de L2, L2 de
 * L3 y L3 de la RAM), no solo leen un solo byte a la vez. Eso seria ineficaz. En su lugar, leen una <i>linea de cache</i>.
 * Una linea de cache generalmente consta de 64 bytes. Por lo tanto, los caches leen 64 bytes a la vez desde caches de nivel
 * inferior o RAM.
 * <p>
 ...
 * <p>
 * Cuando una linea de cache se invalida porque los datos dentro de esa linea de cache han sido cambiados por otra CPU, entonces la
 * linea de cache invalidada debe actualizarse, ya sea desde la cache L3 o mediante mecanismos de coherencia de cache. Por lo tanto,
 * si la CPU necesita leer la linea de cache invalidada, debe esperar hasta que se actualice la linea de cache. Esto da como
 * resultado una degradacion del rendimiento. El tiempo de la CPU se pierde esperando la actualizacion de la linea de cache, lo que
 * significa que la CPU puede ejecutar menos instrucciones durante ese tiempo.
 * <p>
 ...
 * <p>
 * La forma de solucionar un problema de intercambio falso es diseñar su codigo de modo que las diferentes variables utilizadas por
 * diferentes subprocesos no terminen almacenandose dentro de la misma linea de cache de la CPU. Exactamente como hacerlo depende
 * de su codigo concreto, pero almacenar las variables en diferentes objetos es una forma de hacerlo, como lo mostro el ejemplo de
 * la seccion anterior.
 * <h3>Evitar el intercambio falso con la anotacion @Contented</h3>
 * <p>
 * Desde Java 8 y 9, Java tiene la anotacion <i>@Contended</i> que puede rellenar campos dentro de clases con bytes vacios (despues
 * del campo, cuando se almacena en RAM), de modo que los campos dentro de un objeto de esa clase no se almacenen dentro de la misma
 * linea de cache de la CPU. A continuacion se muestra la clase Counter del ejemplo anterior con la anotacion {@code @Contended}
 * agregada a uno de los campos. Agregar esta anotacion hizo que el tiempo de ejecucion se redujera aproximadamente al mismo tiempo
 * que cuando los dos subprocesos usaban dos instancias de Counter diferentes. Aqui esta la clase Counter modificada:
 * <pre>{@code
 ...
 * relleno entre ellos y otros campos de la clase. A continuacion se muestra un ejemplo de agrupacion de campos con la anotacion @Contended:
 * <pre>{@code
 * public class Counter1 {
 ...
 * <p>
 * De forma predeterminada, las anotaciones @Contended agregan 128 bytes de relleno despues de un campo anotado con @Contended. Sin
 * embargo, puede indicarle a la JVM cuantos bytes usar como relleno mediante el argumento de la linea de comando de la JVM. Asi es
 * como se ve el argumento de la linea de comando:
 * <p>
 ...
 * subyacente, es decir, cuantos bytes contiene cada linea de cache de la CPU. Si lo sabe, puede optimizar el relleno de prevencion
 * de intercambio falso para que coincida con el tamaño de la linea de cache. Por lo tanto, si las lineas de cache tienen solo 64
 * bytes, no hay razon para rellenar con 128 bytes. Ademas, si las lineas de cache son de 256 bytes, rellenar con solo 128 bytes no
 * sera suficiente para evitar el intercambio falso.
 * <p>
 * Links:
 * <a href="http://tutorials.jenkov.com/java-concurrency/false-sharing.html">False Sharing in Java</a>
 */
