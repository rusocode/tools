 ...
 * https://www.javatpoint.com/race-condition-in-java#:~:text=Java%20is%20a%20multi%2Dthreaded,condition%20is%20a%20concurrency%20bug.
 */
...
public class RaceConditionDemo implements Runnable {
	...
	public void run() {
		 ...
		 * La sincronización de subprocesos se puede lograr utilizando un bloque sincronizado de código Java.
		 * 
		 * Para evitar condiciones de carrera, el incremento y decremento de la varibale se ejecuta dentro de un bloque
		 * sincronizado. Con esta implementación, solo un único subproceso puede ejecutar la suma al mismo tiempo. */
		synchronized (this) {
			...
			decrement();
			System.out.println("Valor de " + Thread.currentThread().getName() + " despues del decremento " + this.getValue());
	...
	public static void main(String args[]) {
		...
		RaceConditionDemo instance = new RaceConditionDemo();

		Thread t1 = new Thread(instance, "Thread-1");
		Thread t2 = new Thread(instance, "Thread-2");
		Thread t3 = new Thread(instance, "Thread-3");
