...
public class BinaryToDecimal {
	...
	private static int convertBinaryToDecimal(byte[] binary) {
		...
		while (i >= 0) {
			...
			if (binary[i] == 1) {
				...
				decimal += Math.pow(BASE, calcularExponente(i));

				// Le resta 1 al indice del numero binario para trabajar con el indice anterior
				i--;
			}
			// Si el bit es 0, entonces...
			else i--; // Le resta 1 al indice del numero binario para trabajar con el indice anterior
		}
	...
	private static int calcularExponente(int indice) {
		return 7 - indice;
	}
