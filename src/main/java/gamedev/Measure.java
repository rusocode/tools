 ...
 * <p>
 * La llamada a {@code nanoTime()} es relativamente costosa. {@code currentTimeMillis()} se ejecuta en algunos (5-6)
 * relojes de CPU, {@code nanoTime()} depende de la arquitectura subyacente y puede tener mas de 100 relojes de CPU.
 * <p>
 * Para medir cuanto tiempo tarda en ejecutarse un codigo:
 * <pre>{@code
 * long startTime = System.nanoTime();
 * // ... el codigo que se mide ...
 * long estimatedTime = System.nanoTime() - startTime;
 * }</pre>
 * Para comparar el tiempo transcurrido con un tiempo de espera, utilice
 * <pre>{@code if (System.nanoTime() - startTime >= timeoutNanos)}</pre>
 * en vez de <pre>{@code if (System.nanoTime() >= startTime + timeoutNanos)}</pre>
 * debido a la posibilidad de desbordamiento numerico.
 * <p>
 * {@code nanoTime()}, como dice el documento, es un temporizador de precision. {@code currentTimeMillis()} NO ES UN
 ...
 * <p>
 * <i>Otra cosa a mencionar...</i>
 * <p>
 * No es seguro comparar los resultados de las llamadas a {@code nanoTime()} entre diferentes JVM, cada JVM puede tener
 * un tiempo de "origen" independiente.
 * <p>
 * {@code currentTimeMillis()} devolvera el mismo valor (aproximado) entre JVM, porque esta vinculado a la hora del
 * reloj de pared.
 * <br><br>
 * Recursos:
