 ...
 * (cuadros por segundo), aunque tambien podemos referirnos a ellos como Fotogramas por segundo o Imagenes por segundo.
 * A la hora de hablar de estas cifras, nos referimos a ellos como el framerate de un videojuego. Y como su nombre
 * indica, es la cantidad de imagenes consecutivas que se muestran en pantalla por cada segundo mientras jugamos.
 *
 * <p>Cuando estas viendo un video, lo que ves en realidad es una secuencia de fotogramas que pasan a gran velocidad
 * para dar la sensacion de movimiento. Lo que realmente ves son imagenes fijas mostrandose de forma consecutiva, aunque
 ...
 * juego en movimiento.
 *
 * <p>Independientemente de los FPS que este siendo capaz de mostrar tu PC o consola, siempre veras exactamente lo mismo
 * en pantalla. <b>La diferencia esta en la fluidez</b>, que sera mayor con mayores FPS, y te permitira reaccionar
 * antes.
 *
 * <br><br>
 *
 * <h2>FPS y frecuencia de refresco: dos factores a tener cuenta a la hora de optimizar el uso de un monitor</h2>
 * Aunque los hercios de pantalla tengan un relacion casi obligada con los FPS, son una cifra que no tienen nada que ver
 * con estos.
 *
 * <p>No hay que confundir los frames por segundo con la frecuencia de refresco en hercios de una pantalla, que hace
 ...
 *
 * <p>Si tienes un ordenador monstruoso, usar un monitor de 60Hz hara que no puedas beneficiarte de alcanzar 90, 120,
 * 144 o mas fotogramas por segundo. Si, tu grafica puede ejecutar el juego a 144fps, pero tu monitor solo te mostrara
 * 60.
 *
 * <br><br>
 *
 ...
 * preparados para trabajar a 144Hz e incluso algunos tope de gama se atreven y llegan a los 240 Hz. Pero hay que tener
 * en cuenta que <b>un rendimiento optimo pone en juego tambien los FPS y la relacion con los hercios</b>.
 *
 * <p>Si por ejemplo usamos un monitor que funciona a 120Hz con un juego que corre a 60fps. La consola o el PC esta
 ...
 * tirara de un recurso como es la Interpolacion (repite el mismo cuadro en este caso dos veces o cuatro si el monitor
 * funcionara a 240Hz).
 *
 * <p><b>Esta solucion lo que hace es que puede generar <i>stuttering</i></b>, una especie de parpadeo en la pantalla
 * que puede resultar bastante molesto. Esto puede darse si el monitor no cuenta con la capacidad de sincronizacion con
 * la grafica que envia la señal del juego.
 *
 * <p>Luego podria darse el caso inverso, que es que contemos con una <b>pantalla con una menor tasa de refresco que los
 ...
 *
 * <p>Ademas, esta asincronicidad <b>provoca el siempre molesto efecto <i>tearing</i></b> o lo que se traduce en la
 * sensacion de imagen dividida en varias partes, algo que se aprecia al realizar cambios bruscos de camara. Un defecto
 * que algunas marcas intentan corregir con sistemas añadidos al firmware que usan en sus paneles como son los que
 * permiten sincronizar las imagenes entre grafica y pantalla (FreeSync de AMD o G-Sync de Nvidia).
 *
 * <p>En este sentido lo ideal es que un monitor <b>funcione siempre a la misma velocidad de refresco que los FPS</b> a
