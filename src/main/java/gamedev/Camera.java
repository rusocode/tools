 ...
 *
 * <p>Los tiles se dibujan individualmente de izquierda a derecha (forma tradicional) usando un doble for. Esto se hace
 * tan rapido que no se nota. Cada iteracion completa (render), "mueve" el mundo hacia una determinada direccion.
 *
 * <p>Si la camara se "mueve" hacia la derecha (+x), los tiles se desplazan hacia la izquierda restando la posicion
 * original. Si la camara se "mueve" hacia la izquierda (-x), los tiles se desplazan hacia la derecha sumando la
 * posicion original. Esto mismo se aplica para el eje vertical y. Si la camara se "mueve" hacia abajo (+y), los tiles
 ...
 * entidad.
 *
 * <p>Este desplazamiento es necesario aplicarlo tanto para los tiles como para la entidad.
 *
 * <p><i>Nota: No confundir el centro de la pantalla con el centro del mundo.</i>
 *
 * <p>Recursos:
