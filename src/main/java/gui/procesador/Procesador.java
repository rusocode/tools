...
class Lamina extends JPanel {
	...
	public Lamina() {
		 ...
		 * configurarMenu1(fuente, "Arial");
		 * configurarMenu1(fuente, "Courier");
		 * configurarMenu1(fuente, "Verdana");
		 * 
		 * JMenu estilo = new JMenu("Estilo");
		 * configurarMenu1(estilo, "Negrita");
		 * configurarMenu1(estilo, "Cursiva");
		 * 
		 * JMenu tamaño = new JMenu("Tamaño");
		 * configurarMenu1(tamaño, "12");
		 * configurarMenu1(tamaño, "16");
		 * configurarMenu1(tamaño, "18");
		 * configurarMenu1(tamaño, "20"); */
	...
	private void configurarMenu2() {
		...
		/* Le sumo 1 al tamaño de la matriz textoItem por que la instruccion textoItem.length almacena el
		 * maximo desde la primera fila, osea 3 columnas, y en la ultima fila ("12", "16", "18", "20") hay 4 columnas,
		 * lo que ocasionaria un ArrayIndexOutOfBoundsException en el for. */
			...
			for (int j = 0; j < textoItem[i].length; j++) { // textoItem[i].length calcula la cantidad de columnas para la fila i.
				...
				menu[i].add(item[i][j]).addActionListener(new ActionListener() { // fuente.add(arial);
					...
					public void actionPerformed(ActionEvent e) {
						 ...
						 * case "18":
						 * case "20":
						 * area.setFont(new Font(fuente, estilo, Integer.parseInt(e.getActionCommand())));
						 * break;
						 * } */
	...
	private void configurarMenu3() {
		...
		ButtonGroup grupo_tamaños = new ButtonGroup();
		...
		/* Con el metodo setAccelerator asigna un atajo de teclado para el item utilizando la clase KeyStroke, en donde
		 * el metodo getKeyStroke devuelve un objeto KeyStroke para esa clave. */
		...
		grupo_tamaños.add(_12);
		grupo_tamaños.add(_16);
		grupo_tamaños.add(_18);
		grupo_tamaños.add(_20);
		...
		tamaño.add(_12).addActionListener(new StyledEditorKit.FontSizeAction("Tamaño", 12));
		tamaño.add(_16).addActionListener(new StyledEditorKit.FontSizeAction("Tamaño", 16));
		tamaño.add(_18).addActionListener(new StyledEditorKit.FontSizeAction("Tamaño", 18));
		tamaño.add(_20).addActionListener(new StyledEditorKit.FontSizeAction("Tamaño", 20));
	...
	private JButton crearBoton(String ruta) {
		...
		/* OBSERVACION: El metodo add de la clase ToolBar pide como argumento un objeto de tipo Action, sin embargo le pasas
		 * un objeto JButton, ¿por que?
		 * 
		 * RESPUESTA: La respuesta esta en que la clase JToolBar hereda de la clase Container el metodo add que permite como
		 * parametro un
		 * objeto de tipo Component y JButton es tambien de tipo Component por polimorfismo ya que la clase JButton es
		 * descendiente
