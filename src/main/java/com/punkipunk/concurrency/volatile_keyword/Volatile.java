package com.punkipunk.concurrency.volatile_keyword;
 ...
 * <p>
 * Si la variable counter no se declara volatil, no hay garantia de cuando se escribe el valor de la variable counter desde los
 * registros de la CPU a la memoria principal. Esto significa que el valor de la variable counter en el registro de la CPU puede
 * no ser el mismo que en la memoria principal. Esta situacion se ilustra aqui:
 * <p>
 ...
 * <p>
 * La palabra clave volatil esta destinada a abordar problemas de visibilidad de variables. Al declarar volatil la variable
 * counter, todas las escrituras en la variable counter se volveran a escribir en la memoria principal inmediatamente. Ademas,
 * todas las lecturas de la variable counter se leeran directamente desde la memoria principal.
 * <p>
 ...
 * siguiente:
 * <ul>
 * <li>Si el subproceso A escribe en una variable volatil y el subproceso B lee posteriormente la misma variable volatil, todas
 * las
 ...
 * escritos en years y months tambien se escriben en la memoria principal. Pero, que pasaria si la JVM reordenara las
 * instrucciones, asi:
 * <pre>{@code
 * public void update(int years, int months, int days) {
 *     this.days = days;
 ...
 * sucede antes de que los nuevos valores se hayan escrito en months y years. Por lo tanto, los nuevos valores no se hacen
 * visibles correctamente para otros hilos. El significado semantico de las instrucciones reordenadas ha cambiado.
 * <p>
 ...
 * <ul>
 * <li>Las lecturas y escrituras en otras variables no se pueden reordenar para que ocurran despues de una escritura en una
 * variable
 ...
 * volatil que se reordenaran para que ocurran antes de esa escritura en el volatil. Simplemente no al reves. Se permite de
 * despues a antes, pero no de antes a despues.
 ...
 * la memoria principal, existen situaciones donde esta declaracion no es suficiente para asegurar la correcta sincronizacion
 * entre hilos. Volatile es adecuado cuando solo un hilo escribe en la variable compartida o cuando multiples hilos escriben
 * valores que no dependen del valor anterior de la variable. Sin embargo, cuando un hilo necesita leer el valor actual de la
 * variable volatil para calcular su nuevo valor, surge una condicion de carrera. En estos casos, como en el incremento de un
 * contador compartido por multiples hilos, volatile no puede garantizar la consistencia. Por ejemplo, si dos hilos leen
 * simultaneamente un contador con valor 0, lo incrementan a 1 en sus registros CPU, y luego escriben el resultado, se perderia
 * una de las actualizaciones. En tales situaciones, se requieren mecanismos de sincronizacion mas robustos para asegurar la
 * integridad de los datos compartidos. Esta situacion se ilustra en el siguiente diagrama:
 * <p>
 * <img src="volatil no siempre es suficiente.PNG">
 * <p>
 * El hilo 1 y el hilo 2 ahora practicamente no estan sincronizados. El valor real de la variable counter compartido deberia haber
 * sido 2, pero cada uno de los subprocesos tiene el valor 1 para la variable en sus registros de CPU, y en la memoria principal
 * el valor sigue siendo 0. ¡Es un desastre! Incluso si los subprocesos finalmente escriben su valor para la variable de contador
 * compartido en la memoria principal, el valor sera incorrecto.
 ...
 * volatil para eso no es suficiente. Es necesario utilizar synchronized en ese caso para garantizar que la lectura y escritura de
 * la variable sea atomica. Leer o escribir una variable volatil no bloquea la lectura o escritura de subprocesos. Para que esto
 ...
 * reordenamiento de instrucciones, que es una tecnica normal de mejora del rendimiento. Por lo tanto, solo debe utilizar
 * variables volatiles cuando realmente necesite imponer la visibilidad de las variables.
 * <p>
 * En la practica, los valores de registro de la CPU normalmente simplemente se escribiran en la cache L1 de la CPU, que es
 * razonablemente rapida. No es tan rapido como escribir en un registro de la CPU, pero sigue siendo rapido. La sincronizacion
 * desde la cache L1 hasta la cache L2 y L3 y de regreso a la memoria principal (RAM) se realiza mediante chips separados de la
 * CPU (hasta donde tengo entendido), por lo que la CPU no esta cargada con eso.
 * <p>
 * Fuente: <a href="https://jenkov.com/tutorials/java-concurrency/volatile.html">Java Volatile Keyword</a>
 */
