...
/**
 * <p>
 * Una <i>race condition</i> (condicion de carrera) es un problema de concurrencia que puede ocurrir dentro de una seccion
 * critica. Una <i>critical section</i> (seccion critica) es una seccion de codigo que se ejecuta mediante multiples subprocesos y
 * donde la secuencia de ejecucion de los subprocesos marca la diferencia en el resultado de la ejecucion simultanea de la seccion
 * critica.
 * <p>
 ...
 * sus valores individuales, 2 y 3, al valor y escriben el resultado en la memoria. En lugar de 5, el valor que queda en
 * {@code count} sera el valor escrito por el ultimo hilo para escribir su valor. En el caso anterior es el hilo A, pero tambien
 ...
 * asi pasar al bloque de codigo del cuerpo de la declaracion if. Alli, varios subprocesos pueden intentar eliminar el par
 * key-value almacenado para la key "key", pero solo uno de ellos podra hacerlo. El resto obtendra un valor {@code null}, ya que
 * otro hilo ya elimino el par key-value.
 ...
 * <p>
 * Para secciones criticas mas pequeñas, puede funcionar hacer toda la seccion critica un bloque sincronizado. Pero, para
 * secciones criticas mas grandes, puede ser beneficioso dividir la seccion critica en secciones criticas mas pequeñas, para
 * permitir que varios subprocesos ejecuten cada una de ellas una seccion critica mas pequeña. Esto puede disminuir la contencion
 * sobre el recurso compartido y, por lo tanto, aumentar el rendimiento de la seccion critica total.
 * <p>
 ...
 * <p>
 * Links:
 * <a href="https://jenkov.com/tutorials/java-concurrency/race-conditions-and-critical-sections.html">Race Conditions and Critical
 * Sections</a>
 * <a
 * href="https://www.netjstech.com/2015/06/race-condition-in-java-multi-threading.html#:~:text=Race%20condition%20in%20Java%20occurs,thus%20the%20name%20race%20condition">Race
 * Condition in Java Multi-Threading</a>
 *
...
public class RaceCondition implements Runnable {
    ...
    /**
     * <p>
     * Detiene el main thread brevemente, permitiendo que los subprocesos A y B tengan tiempo para ejecutarse y competir entre si
     * (condicion de carrera). Sin esta pausa, el hilo principal podria continuar inmediatamente y leer un valor de 'count' igual
     * a 0, ya que se ejecutaria en paralelo con los subprocesos A y B antes de que estos pudieran modificar la variable
     * compartida.
     * <p>
     * El uso de sleep() simula retrasos que ocurririan en un sistema real con multiples procesos y usuarios. En produccion, el
     * cambio de contexto entre subprocesos competidores es impredecible debido a la carga del sistema. Esto hace que los errores
     * por condiciones de carrera sean dificiles de detectar y reproducir, ya que dependen de patrones de ejecucion especificos
     * que pueden no ocurrir durante las pruebas. La simulacion ayuda a exponer estos problemas potenciales en un entorno
     * controlado.
     */
