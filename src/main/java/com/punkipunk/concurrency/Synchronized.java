 ...
 * el subproceso A esta ejecutando add(), entonces el subproceso B no puede ejecutar ni add() ni subtract() hasta que el
 * subproceso A haya salido de add().
 * <p>
 ...
 * metodos estaticos sincronizados de cada clase. Un hilo por clase independientemente del metodo sincronizado estatico que
 * llame.
 ...
 *         com.punkipunk.log.writeln(msg1);
 *         com.punkipunk.log.writeln(msg2);
 *     }
 *
 ...
 * <p>
 * Si el segundo bloque sincronizado se hubiera sincronizado en un objeto diferente a {@code this}, entonces un hilo a la vez
 * habria podido ejecutarse dentro de cada metodo.
 ...
 * <p>
 * Si el segundo bloque sincronizado se hubiera sincronizado en un objeto diferente a {@code MyClass.class}, entonces un hilo
 * podria ejecutarse dentro de cada metodo al mismo tiempo.
 ...
 * guardada en un registro de CPU por un subproceso se "compromete" en la memoria principal, y no hay garantia sobre cuando otros
 * subprocesos "actualizan" una variable guardada en un registro de CPU desde la memoria principal.
 * <p>
 ...
 * <p>
 * Estas restricciones son similares a las que impone la palabra clave volatile. El objetivo es asegurar que el codigo se comporte
 * de manera predecible en entornos multihilo, evitando que las optimizaciones del compilador alteren la logica prevista del
 * programa. Esto permite a los desarrolladores escribir codigo concurrente con la confianza de que se ejecutara como se espera,
 ...
 * la misma instancia para valores identicos. Esto puede resultar en una sincronizacion no intencionada entre bloques que se
 * suponia eran independientes. Para evitar estos problemas y garantizar un comportamiento predecible, se recomienda sincronizar
 * en {@code this} o en un nuevo objeto creado con {@code new Object()}, ya que Java no los almacena en cache ni los reutiliza
 * internamente.
 ...
 * que tiene una semantica de bloqueo mas avanzada que un bloque sincronizado. Java en realidad viene con una clase
 * {@code ReadWriteLock} incorporada que puedes usar.
 * <p>
 * ¿Que sucede si desea permitir que N subprocesos ingresen a un bloque sincronizado, y no solo a uno? Podrias usar un
 * {@code Semaphore} para lograr ese comportamiento. En realidad, Java viene con la clase Semaphore incorporada.
 * <p>
 ...
 * <p>
 * ¿Que pasa si solo tiene un hilo escribiendo en una variable compartida y otros hilos solo leen esa variable? Entonces es
 * posible que puedas usar una variable volatil sin ninguna sincronizacion.
 ...
 * consistencia de la memoria: si un objeto es visible para mas de un hilo, todas las lecturas o escrituras en las variables de
 * ese objeto se realizan a traves de metodos sincronizados."</i>
 * <p>
 * En pocas palabras: cuando tiene dos subprocesos que leen y escriben en el mismo "recurso", digamos una variable nombrada
 * {@code height}, debe asegurarse de que estos subprocesos accedan a la variable de forma atomica. Sin la palabra clave
 * synchronized, es posible que el hilo A no vea el cambio que se hizo en el hilo B a la variable {@code height} o, lo que es
 * peor, puede que solo se haya cambiado a la mitad. Esto no seria lo que logicamente esperas.
 * <p>
 * Fuente: <a href="https://jenkov.com/tutorials/java-concurrency/synchronized.html">Java Synchronized Blocks</a>
 */
