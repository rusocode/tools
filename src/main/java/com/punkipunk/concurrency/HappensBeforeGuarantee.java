...
/**
 * <p>
 * <i>Java happens before guarantee</i> es un conjunto de reglas que rigen como la JVM y la CPU pueden reordenar las
 * instrucciones para mejorar el rendimiento. La happens before guarantee (sucede antes de la garantia) hace posible que los
 * subprocesos dependan de cuando se sincroniza un valor de variable hacia o desde la memoria principal, y que otras variables se
 * han sincronizado al mismo tiempo. La happens before guarantee se centra en el acceso a variables volatiles y a variables a las
 * que se accede desde bloques sincronizados.
 ...
 * llevar a comportamientos inesperados en programas multihilo, destacando la importancia de considerar estos aspectos en el
 * dise&ntilde;o de sistemas concurrentes.
 ...
 * <p>
 * En esta seccion cubrire brevemente la garantia de visibilidad volatil y explicare como el reordenamiento de instrucciones puede
 * romper la garantia de visibilidad volatil. Es por eso que tambien tenemos la volatil de <i>happens before guarantee</i>, para
 * imponer algunas restricciones en el reordenamiento de instrucciones para que la garantia de visibilidad volatil no se rompa con
 * el reordenamiento de instrucciones.
 ...
 * <li>Todas las variables visibles para el hilo que escribe en la variable volatil tambien se sincronizan con la memoria
 * principal.
 * </ol>
 * <p>
 * En el ejemplo dado:
 ...
 * }</pre>
 * <p>
 * Cuando se escribe en {@code volatileVarC} (declarada como volatil), no solo su valor se guarda en la memoria principal, sino
 * que tambien los valores de nonVolatileVarA y nonVolatileVarB se sincronizan con la memoria principal. Esto garantiza que todos
 * los cambios realizados antes de la escritura volatil sean visibles para otros hilos que lean la variable volatil
 * posteriormente.
 * <h4>La garantia de visibilidad de lectura volatil de Java</h4>
 * <p>
 * Cuando lee el valor de un volatil, se garantiza que el valor se leera directamente desde la memoria. Ademas, todas las
 * variables visibles para el hilo que lee la variable volatil tambien tendran sus valores actualizados desde la memoria
 * principal.
 * <p>
 ...
 * <ol>
 * <li>Cuando hasNewFrame se establece como true, {@code frame} y {@code framesStoredCount} se sincronizan con la memoria
 * principal.
 ...
 * instruccion (porque hasNewFrame es volatil), &iexcl;lo cual es <i>antes</i> de que se les asignen sus nuevos valores!
 ...
 * de esa instruccion en el momento de su ejecucion no se mantendria. Es por eso que las lecturas posteriores no se pueden
 * reordenar para que aparezcan encima de una lectura de una variable volatil.
 * <p>
 ...
 * coloca al comienzo del metodo, para garantizar que todas las variables se actualicen desde la memoria principal antes de
 * leerlas.
 ...
 * bloque sincronizado. Si se reordenara la lectura de una variable para que apareciera antes del comienzo del bloque
 * sincronizado, se perderia la garantia de que los valores de las variables se actualicen desde la memoria principal. Este seria
 * el caso en el caso de la siguiente reordenacion no permitida de las instrucciones:
 * <pre>{@code
 *     public void get(Values v) {
 *         v.valB = this.valB;
 *         v.valA = this.valA;
 ...
 * }</pre>
 * <p>
 * Fuente: <a href="https://jenkov.com/tutorials/java-concurrency/java-happens-before-guarantee.html">Java Happens Before
 * Guarantee</a>
 */
