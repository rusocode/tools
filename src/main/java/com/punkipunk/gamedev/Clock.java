 ...
 * <p>
 * Los procesadores funcionan por ciclos. Por ejemplo, a una calculadora que tiene una lista de operaciones para
 * calcular y devolver el resultado se le llama ciclo.
 * <p>
 * La frecuencia (ciclos por segundo) se regula desde un oscilador de cristal. Este es un componente electronico que
 ...
 * <p>
 * La velocidad del reloj se miden en Hertz. Cuanto mayor sea la frecuencia del reloj, mas operaciones puede realizar
 ...
 * <p>
 * Normalmente el reloj tiene por defecto una frecuencia base de 1 GHz. Eso significa que de base, el reloj manda al
 * procesador a hacer mil millones de operaciones (ciclos) por segundo. Cada ciclo del procesador es un conjunto de
 ...
 * <p>
 * Un <b>tick</b> es una unidad arbitraria para medir el tiempo interno del sistema. Por lo general, hay un contador
 ...
 * <p>
 * El reloj del sistema generalmente se implementa como un temporizador de intervalo programable que interrumpe
 ...
 * <p>
 * Para obtener la hora del sistema en Java se utilizan los metodos {@code currentTimeMillis()} que devuelve el tiempo
 * actual en milisegundos y {@code nanoTime()} que devuelve el tiempo en nanosegundos. La epoca que toma
 * {@code currentTimeMillis()} es desde el <i>1 de enero de 1970</i> y el valor devuelto por {@code nanoTime()} es
 * arbitrario.
 * <br><br>
 * <h2>Diferencia entre el reloj del cpu y el reloj del sitema</h2>
 ...
 * <p>
 * El uso del CPU se utiliza para cuantificar como se comparte el procesador entre los programas de computadora. El
 ...
 * <p>
 * Por el contrario, el tiempo de pared es el tiempo que transcurre desde el inicio de un programa hasta el final
...
public class Clock {
     ...
     * <p>
     * Este valor puede cambiar durante una invocacion particular de la maquina virtual. Por lo tanto, las aplicaciones
     * que son sensibles a la cantidad de procesadores disponibles deben sondear ocasionalmente esta propiedad y ajustar
     * su uso de recursos de manera adecuada.
     * <p>
     * Para obtener los cantidad de procesadores reales del sistema, lo mas probable es que haya que realizar llamadas
     * al sistema operativo utilizando llamadas nativas a traves de Java Native Interface (JNI).
     *
